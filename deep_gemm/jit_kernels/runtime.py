...
class FP8SwapabGemmRuntime(Runtime):
    def __init__(self, path: str) -> None:
        super().__init__(path)
...
#include <deep_gemm/fp8_swapab_gemm.cuh>

using namespace deep_gemm;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&fp8_gemm_kernel_swapab<
        {kwargs['M']},
        {kwargs['K']},
        {kwargs['BLOCK_M']},
        {kwargs['BLOCK_N']},
...
'''
        if int(os.getenv('DG_JIT_DEBUG', 0)):
            print(f'Generated FP8 SwapAB GEMM code:\n{code}')
        return code
