...
def make_2d_tma_d_desc(gemm_type: GemmType, t: torch.Tensor,
    ...
    return make_2d_tma_desc(t,
                            ...
                            block_n if swizzle_mode == 0 else swizzle_mode // t.element_size(), min(block_m, shape_m),
...
class FP8GemmPerTensorRuntime(Runtime):
    def __init__(self, path: str) -> None:
        super().__init__(path)
...
'''
        if int(os.getenv('DG_JIT_DEBUG', 0)):
            print(f'Generated FP8 GEMM per tensor code:\n{code}')
        return code
...
class FP8GemmPerTensorSwapabRuntime(Runtime):
    def __init__(self, path: str) -> None:
        super().__init__(path)
...
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&fp8_gemm_per_tensor_swapab_kernel<
        {kwargs['M']},
        {kwargs['K']},
        {kwargs['BLOCK_M']},
        {kwargs['BLOCK_N']},
        ...
        {kwargs['NUM_MATH_THREADS_PER_GROUP']},
        {kwargs['NUM_TMA_MULTICAST']},
        {'true' if kwargs['IS_TMA_MULTICAST_ON_ACT'] else 'false'},
        GemmType::{kwargs['GEMM_TYPE']}
      >);
}};
'''
        if int(os.getenv('DG_JIT_DEBUG', 0)):
            print(f'Generated FP8 GEMM per tensor SwapAB code:\n{code}')
        return code
    ...
    def launch(kernel: cbd.CUkernel, kwargs: Dict[str, Any]) -> cbd.CUresult:
        ...
        arg_values = (
            kwargs['OUT_PUT'].data_ptr(),
            kwargs['SCALES_A'].data_ptr(),
            kwargs['GROUPED_LAYOUT'].data_ptr(),
            kwargs['N'],
            kwargs['TENSOR_MAP_A'],
            kwargs['TENSOR_MAP_B'],
