...
from .runtime import (
    FP8GemmRuntime, FP8GemmPerTensorRuntime, FP8GemmPerTensorSwapabRuntime,GemmType,
...
def m_grouped_gemm_fp8_fp8_bf16_nt_contiguous_per_tensor(lhs: Tuple[torch.Tensor, torch.Tensor],
                                              rhs: Tuple[torch.Tensor, torch.Tensor],
                                              out: torch.Tensor, m_indices: torch.Tensor) -> None:
    """
    Perform a grouped GEMM (contiguous format) with FP8 inputs and BF16 output, with 1x128 LHS scaling and 128x128 RHS scaling.
    ...
    num_sms = get_num_sms()
    num_sms, block_m, block_n, num_stages, tma_multicast_config, smem_config = get_best_configs(
        m, n, k, 1, num_sms, is_grouped_contiguous=True, is_per_tensor=True)
    block_k = 128
    num_tma_threads = 128
    num_math_threads_per_group = 128
    ...
    # Generate, build and run the kernel
    code = FP8GemmPerTensorRuntime.generate(kwargs)
    runtime = build('m_grouped_gemm_fp8_fp8_bf16_nt_per_tensor', code, FP8GemmPerTensorRuntime, kwargs)
    runtime(**kwargs)
...
def m_grouped_gemm_fp8_fp8_bf16_nt_masked_per_tensor(lhs: Tuple[torch.Tensor, torch.Tensor],
                                          rhs: Tuple[torch.Tensor, torch.Tensor],
                                          out: torch.Tensor, masked_m: torch.Tensor, expected_m: int) -> None:
    """
    Perform a grouped GEMM (masked format) with FP8 inputs and BF16 output, with 1x128 LHS scaling and 128x128 RHS scaling.
    ...
    num_sms = get_num_sms()
    num_sms, block_m, block_n, num_stages, tma_multicast_config, smem_config = get_best_configs(
        expected_m, n, k, num_groups, num_sms, is_grouped_masked=True, is_per_tensor=True)
    ...
    # Generate, build and run the kernel
    code = FP8GemmPerTensorRuntime.generate(kwargs)
    runtime = build('m_grouped_gemm_per_tensor_fp8_fp8_bf16_nt', code, FP8GemmPerTensorRuntime, kwargs)
    runtime(**kwargs)

def m_grouped_gemm_fp8_fp8_bf16_nt_masked_per_tensor_swapab(lhs: Tuple[torch.Tensor, torch.Tensor],
                                          rhs: Tuple[torch.Tensor, torch.Tensor],
    ...
    assert lhs_scales.shape == (num_groups,)
    assert rhs_scales.shape == (1,)
    assert lhs.dtype == torch.float8_e4m3fn and lhs_scales.dtype == torch.float32
    assert rhs.dtype == torch.float8_e4m3fn and rhs_scales.dtype == torch.float32
    assert out.dtype == torch.bfloat16
    ...
    num_sms = get_num_sms()
    num_sms, block_m, block_n, num_stages, tma_multicast_config, smem_config = get_best_configs(
        m, expected_n, k, num_groups, num_sms, is_grouped_masked=True, is_per_tensor=True, is_swapab=True)
    # # Extra checks for TMA store
    # if num_groups > 1 and n > block_n:
    #     assert n % block_n == 0, f'For masked grouped GEMM, shape N should be multiple of the block N (current block N: {block_n})'
    ...
    # Generate, build and run the kernel
    code = FP8GemmPerTensorSwapabRuntime.generate(kwargs)
    runtime = build('m_grouped_gemm_per_tensor_fp8_fp8_bf16_nt_swapab', code, FP8GemmPerTensorSwapabRuntime, kwargs)
    runtime(**kwargs)
