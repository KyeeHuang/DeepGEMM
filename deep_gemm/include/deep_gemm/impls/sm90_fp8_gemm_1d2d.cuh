...
template <uint32_t SHAPE_M, uint32_t SHAPE_N, uint32_t SHAPE_K,
          ...
          uint32_t kNumSMs, GemmType kGemmType>
__global__ __launch_bounds__(kNumTMAThreads + kNumMathThreads, 1) void
sm90_fp8_gemm_per_tensor_1d2d_impl(float* scales_b, int* grouped_layout,
                        uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_a,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_b,
                        const __grid_constant__ cute::TmaDescriptor tensor_map_d) {
    ...
    // Configs
    constexpr uint32_t kFullKOfAllStages = kNumStages * BLOCK_K;
    constexpr uint32_t num_iterations = ceil_div(SHAPE_K, kFullKOfAllStages);
    const uint32_t warp_idx = __shfl_sync(0xffffffff, threadIdx.x / 32, 0);
    ...
    // Block scheduler
    uint32_t m_block_idx, n_block_idx;
    auto scheduler = Scheduler<kGemmType, SHAPE_N, BLOCK_M, BLOCK_N, kNumGroups, kNumTMAMulticast, kIsTMAMulticastOnA>(shape_m, shape_n, grouped_layout);
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            // }
            int current_group_idx = scheduler.curr_group_idx;
            if constexpr (kGemmType == GemmType::GroupedContiguous) {
                current_group_idx = __ldg(scheduler.grouped_layout + m_block_idx * BLOCK_M);
            }
            auto local_scales_b = scales_b + curr_group_idx;
