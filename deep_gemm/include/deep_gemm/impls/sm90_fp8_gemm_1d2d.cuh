...
sm90_fp8_gemm_per_tensor_1d2d_impl(float* scales_b, int* grouped_layout,
    ...
    static constexpr uint32_t SMEM_B_SIZE_PER_STAGE = BLOCK_N * BLOCK_K * sizeof(__nv_fp8_e4m3);
    const uint32_t& shape_k_scales = ceil_div(shape_k, BLOCK_K);
    const uint32_t& smem_sfb_size = align<uint32_t>(1 * (kMustUseUniformedScaleB ? 1 : 2) * sizeof(float), sizeof(Barrier));
    ...
    // Fill barriers
    auto barrier_start_ptr = reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_scales_b) + smem_sfb_size);
    #pragma unroll
    for (uint32_t i = 0; i < kNumStages; ++ i) {
        full_barriers[i] = barrier_start_ptr + i;
        empty_barriers[i] = barrier_start_ptr + kNumStages + i;
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            uint32_t num_scales_b = 1 * (num_former_iters >= num_full_iters ? 1 : 2);
