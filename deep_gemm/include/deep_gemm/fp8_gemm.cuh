...
template <uint32_t SHAPE_N, uint32_t SHAPE_K,
          ...
          GemmType kGemmType>
__global__ void __launch_bounds__(get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
fp8_gemm_per_tensor_kernel(float* scales_b, int* grouped_layout,
                uint32_t shape_m,
                const __grid_constant__ CUtensorMap tensor_map_a,
                const __grid_constant__ CUtensorMap tensor_map_b,
                const __grid_constant__ CUtensorMap tensor_map_d) {
    ...
    static constexpr uint32_t SMEM_B_SIZE_PER_STAGE = BLOCK_N * BLOCK_K * sizeof(__nv_fp8_e4m3);
    static constexpr uint32_t SHAPE_K_SCALES = ceil_div(SHAPE_K, BLOCK_K);
    static constexpr uint32_t SMEM_SCALES_B_SIZE = ceil_div<uint32_t>(1 * sizeof(float), sizeof(Barrier)) * sizeof(Barrier);
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            launch_k_iterations([&](uint32_t k_iter, auto divisible_type, auto skip_type, auto _) {
                ...
                // float scale_b_0 = ld_shared(smem_scales_b);
                #pragma unroll
                for (uint32_t s = 0; s < kNumInnerStages; ++ s) {
                    // Wait TMA arrivals
                    full_barriers[s]->wait((scheduler.current_iter * kNumIterations + k_iter) & 1);
                    ...
                    for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M; ++ local_idx) {
                    	...
                    	for (uint32_t i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                            ...
                            shifted_accum[i * 4 + 2] += scale_b_0 * accum[i * 4 + 2];
                            shifted_accum[i * 4 + 3] += scale_b_0 * accum[i * 4 + 3];
                    	}
                    }
                }
            ...
            }, not scheduler.is_computation_valid(m_block_idx, math_wg_idx * WGMMA::M), 0);
...
template <uint32_t SHAPE_M, uint32_t SHAPE_K,
          ...
          GemmType kGemmType>
__global__ void __launch_bounds__(get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
fp8_gemm_per_tensor_swapab_kernel(__nv_bfloat16* gmem_d, float* scales_a, int* grouped_layout,
                uint32_t shape_n,
    ...
    DG_STATIC_ASSERT(kGemmType == GemmType::GroupedMasked, "SwapAB GEMM only supports grouped masked");
    // Types
    using WGMMA = typename FP8MMASelector<BLOCK_N>::type;
    using Barrier = cutlass::arch::ClusterTransactionBarrier;
    DG_STATIC_ASSERT(BLOCK_M % WGMMA::M == 0, "Invalid block size");
    ...
    static constexpr uint32_t SMEM_B_SIZE_PER_STAGE = BLOCK_N * BLOCK_K * sizeof(__nv_fp8_e4m3);
    static constexpr uint32_t SHAPE_K_SCALES = ceil_div(SHAPE_K, BLOCK_K);
    static constexpr uint32_t SMEM_SCALES_A_SIZE = ceil_div<uint32_t>(1 * sizeof(float), sizeof(Barrier)) * sizeof(Barrier); // renamed to A (weight scales)
    ...
    smem_scales_a = reinterpret_cast<float*>(smem_buffer + SMEM_D_SIZE + kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE));
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        if (threadIdx.x == kNumMathThreads) {
            ...
            while (scheduler.get_next_block(n_block_idx, m_block_idx)) {
                ...
                launch_k_iterations([&](uint32_t k_iter, auto divisible_type, auto _, auto __) {
                    ...
                    // NOTES: there may be additional odd rows/columns or cases where multicast is not possible.
                    const bool is_tma_multicast_valid = scheduler.is_tma_multicast_valid(m_block_idx);
                    const uint32_t num_tma_multicast_a = (kIsTMAMulticastOnAct and is_tma_multicast_valid) ? kNumTMAMulticast : 1;
                    const uint32_t num_tma_multicast_b = (not kIsTMAMulticastOnAct and is_tma_multicast_valid) ? kNumTMAMulticast : 1;
                    DG_STATIC_ASSERT(kNumTMAMulticast <= 2, "Scheduler does not support > 2 TMA multicast");
        ...
        while (scheduler.get_next_block(n_block_idx, m_block_idx)) {
            ...
            // NOTES: except the first warp, we want to overlap loading B scales with TMA stores between tasks
            // if (threadIdx.x == 32) {
            //     auto local_scales_a = scales_a + scheduler.curr_group_idx;
            //     st_shared(smem_scales_a, __ldg(local_scales_a));
            // }
            int curren_group_idx = scheduler.curr_group_idx;
            auto local_scales_a = scales_a + curren_group_idx;
            float scale_a_0 = __ldg(local_scales_a);
            // cutlass::arch::NamedBarrier(kNumMathThreads).sync();
            ...
            // Launch MMAs
            launch_k_iterations([&](uint32_t k_iter, auto divisible_type, auto skip_type, auto _) {
            // launch_k_iterations([&](int k_iter, auto divisible_type) {
                constexpr bool kSkipComputation = std::is_same_v<decltype(skip_type), SkipComputation>;
                constexpr bool kHasDivisibleStages = std::is_same_v<decltype(divisible_type), DivisibleK>;
                // constexpr uint32_t kNumInnerStages = kHasDivisibleStages ? kNumStages : (SHAPE_K % kFullKOfAllStages) / BLOCK_K;
                constexpr uint32_t kNumInnerStages = kSkipComputation ? 0 :
                ...
                // float scale_a_0 = ld_shared(smem_scales_a);
                #pragma unroll
                for (uint32_t s = 0; s < kNumInnerStages; ++ s) {
                    // Wait TMA arrivals
                    full_barriers[s]->wait((scheduler.current_iter * kNumIterations + k_iter) & 1);
                    ...
                    for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M; ++ local_idx) {
                        ...
                        #pragma unroll
                    	for (uint32_t i = 0; i < WGMMA::kNumAccum; ++ i) {
                            shifted_accum[i] += scale_a_0 * accum[i];
                    	}
                    }
                }
            ...
            }, not scheduler.is_computation_valid(n_block_idx, 0), 0);
            // });
            ...
            for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M; ++ local_idx) {
                ...
                for (auto i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                    ...
                    if (kSwizzleDMode == 128 && !cross_boundary) {
                        #pragma unroll
                        for (auto i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                            uint8_t* smem_ptr = nullptr;
