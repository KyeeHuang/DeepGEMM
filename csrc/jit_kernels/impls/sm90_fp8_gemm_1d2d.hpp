...
class SM90FP8GemmPerTensor1D2DRuntime final: public LaunchRuntime<SM90FP8GemmPerTensor1D2DRuntime> {
public:
    struct Args {
        int m, n, k, num_groups;
        const std::string& compiled_dims;
...
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm90_fp8_gemm_per_tensor_1d2d_impl<
        {}, {}, {},
        {},
        {}, {}, {},
        {},
...
static void sm90_m_grouped_fp8_gemm_contiguous_per_tensor_1d2d(const torch::Tensor& a,
                                                    const torch::Tensor& b, const torch::Tensor& sfb,
                                                    const torch::Tensor& d,
                                                    const torch::Tensor& m_indices,
                                                    const int& num_groups, const int& m, const int& n, const int& k,
    ...
    const auto& config = get_best_config<SM90ArchSpec>(
        ...
        m, n, k, 1, major_a, major_b,
        torch::kFloat8_e4m3fn, d.scalar_type(), false,
        device_runtime->get_num_sms(), true);
    ...
    const auto& code = SM90FP8GemmPerTensor1D2DRuntime::generate(args);
    const auto& runtime = compiler->build("sm90_m_grouped_fp8_gemm_contiguous_per_tensor_1d2d", code);
    SM90FP8GemmPerTensor1D2DRuntime::launch(runtime, args);
}
...
static void sm90_m_grouped_fp8_gemm_masked_per_tensor_1d2d(const torch::Tensor& a,
                                                const torch::Tensor& b, const torch::Tensor& sfb,
                                                const torch::Tensor& d,
                                                const torch::Tensor& masked_m,
                                                const int& num_groups, const int& m, const int& n, const int& k,
    ...
    const auto& code = SM90FP8GemmPerTensor1D2DRuntime::generate(args);
    const auto& runtime = compiler->build("sm90_fp8_m_grouped_gemm_masked_per_tensor_1d2d", code);
    SM90FP8GemmPerTensor1D2DRuntime::launch(runtime, args);
}
